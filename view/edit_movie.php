...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $poster_url = $movie['Poster_url']; // Default ke nilai yang ada
        ...
        if (in_array($fileType, $allowedTypes) && $fileSize <= $maxSize) {
            ...
            if (move_uploaded_file($fileTmpPath, $destPath)) {
                ...
                if (strpos($movie['Poster_url'], 'default_poster.jpg') === false) {
                    $oldFile = str_replace('http://' . $_SERVER['HTTP_HOST'] . '/movlix/public/uploads/posters/', $uploadDir, $movie['Poster_url']);
            ...
            $error = 'Invalid file type or size. Allowed: JPG, PNG, JPEG, max 2MB';
        }
    }

    // Validasi input
    if (empty($error)) {
        if (empty($title) || empty($genre_id) || empty($release_year)) {
...
<head>
    ...
    <style>
        /* Gunakan style yang sama dengan add_movie.php */
        ...
        .btn-submit { background: #d32f2f; color: white; border: none; padding: 12px 20px; border-radius: 5px; cursor: pointer; font-size: 16px; transition: background 0.3s; width: 100%; }
        ...
        .poster-preview { max-width: 200px; max-height: 300px; border-radius: 5px; border: 2px solid #444; margin: 0 auto; }
    </style>
</head>
                ...
                <select id="genre_id" name="genre_id" class="form-control" required>
                    ...
                    <?php foreach ($genres as $genre): ?>
                        <option value="<?= $genre['ID_Genre'] ?>" 
                            <?= $movie['Genre_id'] == $genre['ID_Genre'] ? 'selected' : '' ?>>
